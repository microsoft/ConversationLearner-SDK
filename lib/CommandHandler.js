...
var CommandHandler = (function () {
    ...
    CommandHandler.CueCommand = function (context, command, args, cb) {
        return tslib_1.__awaiter(this, void 0, void 0, function () {
            var cueCommand, memory, card, card, card, card, action, card, entity, type, card, error_1, errMsg;
            return tslib_1.__generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 2:
                        ...
                        return [3 /*break*/, 8];
                    case 3:
                        if (!(command == Consts_1.Commands.CREATEAPP)) return [3 /*break*/, 4];
                        card = Utils_1.Utils.MakeHero("Create App", '{appName} {LUIS key}', "Enter new App name and Luis Key", null);
                        cb([card]);
                        return [3 /*break*/, 8];
                    case 4:
                        if (!(command == Consts_1.Commands.EDITACTION)) return [3 /*break*/, 6];
                    ...
                    case 5:
                        ...
                        return [3 /*break*/, 8];
                    case 6:
                        if (!(command == Consts_1.Commands.EDITENTITY)) return [3 /*break*/, 8];
    ...
    CommandHandler.HandleIntCommand = function (context, input, cb) {
        ...
        if (context.state[Consts_1.UserStates.TEACH]) {
        ...
        else if (command == Consts_1.IntCommands.ADDENTITY) {
            this.CueCommand(context, Consts_1.Commands.ADDENTITY, null, function (responses) {
                cb(responses);
            });
        }
        else if (command == Consts_1.IntCommands.ADDRESPONSE) {
            ...
            BlisApp_1.BlisApp.Delete(context, arg, function (text) {
                cb([text]);
            });
        }
        else if (command == Consts_1.IntCommands.DELETEDIALOG) {
            TrainDialog_1.TrainDialog.Delete(context, arg, function (text) {
                cb([text]);
            });
        }
    ...
    CommandHandler.ProcessCommand = function (context, command, args, cb) {
        ...
        if (command == Consts_1.Commands.ACTIONS) {
            Action_1.Action.Get(context, null, args, function (responses) {
                cb(responses);
            });
        }
        if (command == Consts_1.Commands.ADDENTITY) {
            ...
            Action_1.Action.Get(context, Consts_1.ActionTypes.API, args, function (responses) {
                cb(responses);
            });
        }
            ...
            Action_1.Action.Get(context, Consts_1.ActionTypes.TEXT, args, function (responses) {
                cb(responses);
            });
        }
            ...
            if (command == Consts_1.Commands.ADDAPICALL) {
                Action_1.Action.Add(context, null, Consts_1.ActionTypes.API, args, function (responses) {
                    cb(responses);
                });
            }
            else if (command == Consts_1.Commands.ADDRESPONSE) {
                Action_1.Action.Add(context, null, Consts_1.ActionTypes.TEXT, args, function (responses) {
                    cb(responses);
                });
            }
            else if (command == Consts_1.Commands.APPS) {
                BlisApp_1.BlisApp.GetAll(context, args, function (text) {
                    cb(text);
                });
            }
                ...
                BlisApp_1.BlisApp.DeleteAll(context, function (text) {
                    cb([text]);
                });
            }
            else if (command == Consts_1.Commands.DELETEACTION) {
                var actionId = args.split(' ')[0];
                Action_1.Action.Delete(context, actionId, function (text) {
                    cb([text]);
                });
            }
                ...
                BlisApp_1.BlisApp.Delete(context, appid, function (text) {
                    cb([text]);
                });
            }
                ...
                BlisAppContent_1.BlisAppContent.Export(context, function (text) {
                    cb([text]);
                });
            }
                ...
                var appId = args.split(' ')[0];
                BlisAppContent_1.BlisAppContent.Import(context, appId, function (text) {
                    cb([text]);
                });
            }
                ...
                var appId = args.split(' ')[0];
                BlisAppContent_1.BlisAppContent.Load(context, appId, function (text) {
                    cb([text]);
                });
            }
                ...
                BlisSession_1.BlisSession.NewSession(context, true, function (results) {
                    cb(results);
                });
            }
            else if (command == Consts_1.Commands.TRAINDIALOGS) {
                var search = args.split(' ')[0];
                TrainDialog_1.TrainDialog.Get(context, search, function (text) {
                    cb(text);
                });
            }
            else {
