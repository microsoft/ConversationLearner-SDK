...
var CommandHandler_2 = require("./CommandHandler");
...
var BlisRecognizer = (function () {
    ...
    BlisRecognizer.prototype.TakeTurnCallback = function (session, ttResponse, error) {
        ...
        if (error) {
            var responses_1 = Menu_1.Menu.AddEditApp(session.context, [error]);
        ...
        if (ttResponse.mode == Consts_1.TakeTurnModes.TEACH) {
            ...
            if (inTeach) {
                ...
                responses.push(Utils_1.Utils.MakeHero('Trained Response:', outText + "\n\n", "Type next user input for this Dialog or", { "Dialog Complete": CommandHandler_1.IntCommands.DONETEACH }));
            }
            else {
    ...
    BlisRecognizer.prototype.ProcessLabelAction = function (session, ttResponse, responses) {
        ...
        responses.push(Utils_1.Utils.MakeHero(title, null, body, {
            "Add Response": CommandHandler_1.IntCommands.ADDRESPONSE,
            "Add API": CommandHandler_1.IntCommands.ADDAPICALL
        }));
        var choices = {};
        ...
        try {
            ...
            this.LoadUser(address_1, function (responses, context) {
                ...
                if (reginput.message.text) {
                    ...
                    // HELP
                    if (Help_1.BlisHelp.IsHelpCommand(userInput)) {
                    ...
                    if (memory.CueCommand()) {
                        CommandHandler_2.CommandHandler.HandleCueCommand(context, userInput, function (responses, teachAction, actionData) {
                            _this.ProcessResult(session, responses, teachAction, actionData);
                        });
                    }
                    else if (CommandHandler_2.CommandHandler.IsCommandLine(userInput)) {
                        CommandHandler_2.CommandHandler.HandleCommandLine(context, userInput, function (responses, teachAction, actionData) {
                            _this.ProcessResult(session, responses, teachAction, actionData);
                        });
                    }
                    else if (CommandHandler_2.CommandHandler.IsIntCommand(userInput)) {
                        CommandHandler_2.CommandHandler.HandleIntCommand(context, userInput, function (responses, teachAction, actionData) {
                            _this.ProcessResult(session, responses, teachAction, actionData);
                        });
                    }
