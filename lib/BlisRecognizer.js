    ...
    BlisRecognizer.prototype.HandleCommand = function (input, address, userState, cb) {
        ...
        if (userState[Consts_1.UserStates.TEACH] && (command != Consts_1.Commands.DUMP) && (command != "!debug") && (command != Consts_1.Commands.TEACH)) {
            if (command == Consts_1.Commands.SAVETRAIN) {
                ...
                msg += dialog.toString();
                var card = this.MakeHero("", "", "Does this look good?", { "Save": Consts_1.Commands.SAVETRAIN, "Abandon": Consts_1.Commands.FORGETTRAIN });
                this.SendResult(address, userState, cb, msg, card);
            }
            else {
                this.SendResult(address, userState, cb, "_In teaching mode. The only valid command is_ " + Consts_1.Commands.DONETRAIN, null);
            }
        }
        else {
                        ...
                        _this.blisClient.TakeTurn(userState, userInput, function (response) {
                            ...
                            if (response.mode == Consts_1.TakeTurnModes.TEACH) {
                                ...
                                if (inTeach_1) {
                                    ...
                                    card = _this.MakeHero('Trained Response:', outText, "Type next user input for this Dialog or", { "Done Training": Consts_1.Commands.DONETRAIN, "New Dialog": Consts_1.Commands.TEACH });
                                }
                                else {
                                    msg = outText;
