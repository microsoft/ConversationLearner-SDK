                ...
                if (reginput.message.text) {
                    ...
                    var ProcessLabelAction_1 = function (ttResponse, responses) {
                        ...
                        if (ttResponse.teachLabelActions.length > 0) {
                            ...
                            for (var i in ttResponse.teachLabelActions) {
                                ...
                                if (!that_1.IsInternalApi(labelAction.content)) {
                            ...
                            memory_1.RememberLastStep(Consts_1.SaveStep.CHOICES, choices);
                        }
                    };
                    ...
                    if (memory_1.CueCommand()) {
                        ...
                        if (inTeach_1) {
                            ...
                            var choices = memory_1.LastStep(Consts_1.SaveStep.CHOICES);
                            if (choices && Object.keys(choices).length > 0) {
                                if (!choices[userInput_1]) {
                                    var msg = "Please select one of the action from above or click 'Add Action' for a new Action";
                                    _this.SendResult(context, cb, [msg]);
                                    return;
                                }
                                userInput_1 = choices[userInput_1];
                                memory_1.RememberLastStep(Consts_1.SaveStep.CHOICES, null);
                            }
                        }
                        else {
                            memory_1.RememberLastStep(Consts_1.SaveStep.INPUT, userInput_1);
