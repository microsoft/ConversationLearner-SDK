...
var BlisRecognizer = (function () {
    ...
    BlisRecognizer.prototype.HandleCommand = function (input, address, userState, cb) {
        ...
        if (userState[Consts_1.UserStates.TEACH] && (command != Consts_1.Commands.DUMP) && (command != "!debug") && (command != Consts_1.Commands.TEACH)) {
            ...
            if (command == Consts_1.Commands.ACTIONS) {
            ...
            else if (command == Consts_1.IntCommands.DONETRAIN) {
                ...
                this.NewSession(userState, false, function (text, card) {
                    _this.SendResult(address, userState, cb, text, card);
                });
            }
                ...
                if (memory.HasEntities()) {
                    ...
                    this.NewSession(userState, true, function (text, card) {
                        _this.SendResult(address, userState, cb, text, card);
                    });
                }
                else {
                    var card = this.MakeHero("", "", "First define some Entities", { "Help": Help_1.Help.NEWAPP });
                    this.SendResult(address, userState, cb, null, card);
                }
            }
    ...
    BlisRecognizer.prototype.HandleIntCommand = function (input, address, userState, cb) {
        ...
        if (userState[Consts_1.UserStates.TEACH]) {
            if (command == Consts_1.IntCommands.SAVETRAIN) {
                ...
                this.EndSession(userState, function (text) {
                    _this.SendResult(address, userState, cb, null, card_1);
                });
            }
            else if (command == Consts_1.IntCommands.FORGETTRAIN) {
                ...
                this.EndSession(userState, function (text) {
                    _this.SendResult(address, userState, cb, null, card_2);
                });
            }
            else if (command == Consts_1.IntCommands.DONETRAIN) {
                var memory = new BlisMemory_1.BlisMemory(userState);
                ...
                var card = this.MakeHero("", "", "Does this look good?", { "Save": Consts_1.IntCommands.SAVETRAIN, "Abandon": Consts_1.IntCommands.FORGETTRAIN });
                this.SendResult(address, userState, cb, msg, card);
            }
            else {
                this.SendResult(address, userState, cb, "_In teaching mode. The only valid command is_ " + Consts_1.IntCommands.DONETRAIN, null);
            }
        }
        else {
            var text = "_Not a valid command._\n\n\n\n" + this.Help(null);
            ...
            if (context && context.message && context.message.text) {
                ...
                this.LoadUser(address_1, function (error, userState) {
                    ...
                    if (userInput.startsWith('!')) {
                        ...
                        _this.HandleIntCommand(userInput, address_1, userState, cb);
                    }
                    else if (userInput.startsWith('#')) {
                            ...
                            if (response.mode == Consts_1.TakeTurnModes.TEACH) {
                                if (response.teachStep == Consts_1.TeachStep.LABELENTITY) {
                                    ...
                                    if (response.teachError) {
                                        ...
                                        memory_1.SaveTrainStep(Consts_1.SaveStep.INPUT, userInput);
                                    }
                                    ...
                                    if (response.teachLabelEntities.length == 0) {
                                        ...
                                        var cardtext = "Click Correct if entities are valid or indicate entities in input string";
                                    ...
                                    card = _this.MakeHero(null, null, cardText, { "Help": Help_1.Help.ADDACTION });
                                }
                                else {
                                ...
                                if (inTeach_1) {
                                    ...
                                    card = _this.MakeHero('Trained Response:', outText, "Type next user input for this Dialog or", { "Dialog Complete": Consts_1.IntCommands.DONETRAIN });
                                }
                                else {
                                    msg = outText;
