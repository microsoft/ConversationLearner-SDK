...
var BlisRecognizer = (function () {
    ...
    BlisRecognizer.prototype.RemoveInternalAPIs = function (labelActions) {
        var outActions = [];
        var sumScore = 0;
        for (var i in labelActions) {
        ...
        if (sumScore <= 0)
            return outActions;
        for (var _i = 0, outActions_1 = outActions; _i < outActions_1.length; _i++) {
            var labelAction = outActions_1[_i];
    ...
    BlisRecognizer.prototype.ProcessLabelAction = function (session, ttResponse, responses) {
        ...
        if (!entities && suggestedEntity && !memory.EntityValue(suggestedEntity)) {
            var saveActionId = memory.SaveLookup(suggestedEntity);
            if (saveActionId) {
                ...
                var errCard = Utils_1.Utils.ErrorCard(null, "Missing Save function for " + suggestedEntity);
                Utils_1.Utils.SendMessage(session.context, errCard);
            }
        }
        memory.RememberTrainStep(Consts_1.SaveStep.ENTITY, memory.DumpEntities());
        ...
        if (ttResponse.teachLabelActions.length > 0) {
            ...
            var labelActions = this.RemoveInternalAPIs(ttResponse.teachLabelActions);
            var msg = "";
            var displayIndex = 1;
            for (var i in labelActions) {
                var labelAction = labelActions[i];
                    ...
                    if (Command_1.Command.IsCueCommand(userInput)) {
                        ...
                        if (context.state[Consts_1.UserStates.TEACH]) {
                            ...
                            if (choices && Object.keys(choices).length > 0) {
                                ...
                                _this.TakeTurn(session, null, actionId);
                            }
                            else {
