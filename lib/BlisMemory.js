...
var BlisMemory = (function () {
    function BlisMemory(session) {
        this.memory = {};
        this.memory = session.userData.Blis;
    }
    /** Clear memory associated with a session */
    BlisMemory.prototype.EndSession = function () {
        this.memory[Consts_1.UserStates.SESSION] = null;
        this.memory[Consts_1.UserStates.TEACH] = false;
        this.memory[Consts_1.UserStates.LASTSTEP] = null;
        this.memory[Consts_1.UserStates.MEMORY] = {};
    };
    /** Init memory for a session */
    BlisMemory.prototype.StartSession = function (sessionId, inTeach) {
        this.EndSession();
        this.memory[Consts_1.UserStates.SESSION] = sessionId;
        this.memory[Consts_1.UserStates.TEACH] = inTeach;
    };
    //--------------------------------------------------------
    // Entity Lookups
    //---------------------------------------------------------
    BlisMemory.prototype.AddEntityLookup = function (entityName, entityId) {
        this.memory[Consts_1.UserStates.ENTITYLOOKUP][entityName] = entityId;
    };
    BlisMemory.prototype.RemoveEntityLookup = function (entityName) {
        try {
            this.memory[Consts_1.UserStates.ENTITYLOOKUP][entityName] = null;
        }
        catch (error) {
    ...
    BlisMemory.prototype.HasEntities = function () {
        return (this.memory[Consts_1.UserStates.ENTITYLOOKUP] && Object.keys(this.memory[Consts_1.UserStates.ENTITYLOOKUP]).length > 0);
    };
    BlisMemory.prototype.EntityValue = function (entityName) {
        var entityId = this.EntityName2Id(entityName);
        var value = this.memory[Consts_1.UserStates.MEMORY][entityId];
    ...
    BlisMemory.prototype.EntityName2Id = function (name) {
        try {
            ...
            name = name.replace('$', '');
            return this.memory[Consts_1.UserStates.ENTITYLOOKUP][name];
        }
        catch (error) {
            BlisDebug_1.BlisDebug.Error(error);
        }
    ...
    BlisMemory.prototype.EntityId2Name = function (id) {
        try {
            for (var name_1 in this.memory[Consts_1.UserStates.ENTITYLOOKUP]) {
                var foundId = this.memory[Consts_1.UserStates.ENTITYLOOKUP][name_1];
                if (foundId == id) {
                    return name_1;
                }
            }
    ...
    BlisMemory.prototype.EntityIds2Names = function (ids) {
        ...
        try {
            for (var _i = 0, ids_1 = ids; _i < ids_1.length; _i++) {
                var id = ids_1[_i];
                var found = false;
                for (var name_2 in this.memory[Consts_1.UserStates.ENTITYLOOKUP]) {
                    var foundId = this.memory[Consts_1.UserStates.ENTITYLOOKUP][name_2];
                    if (foundId == id) {
                        names.push(name_2);
                        found = true;
                    }
    ...
    BlisMemory.prototype.RememberEntityById = function (entityId, entityValue) {
        try {
            ...
            if (entityName.startsWith(Consts_2.ActionCommand.BUCKET)) {
                if (!this.memory[Consts_1.UserStates.MEMORY][entityId]) {
                    this.memory[Consts_1.UserStates.MEMORY][entityId] = [];
                }
                this.memory[Consts_1.UserStates.MEMORY][entityId].push(entityValue);
            }
            else {
                this.memory[Consts_1.UserStates.MEMORY][entityId] = entityValue;
            }
        }
        catch (error) {
            BlisDebug_1.BlisDebug.Error(error);
    ...
    BlisMemory.prototype.RememberEntity = function (entity) {
        try {
            ...
            if (entity.metadata && entity.metadata.bucket) {
                if (!this.memory[Consts_1.UserStates.MEMORY][entity.id]) {
                    this.memory[Consts_1.UserStates.MEMORY][entity.id] = [];
                }
                this.memory[Consts_1.UserStates.MEMORY][entity.id].push(entity.value);
            }
            else {
                this.memory[Consts_1.UserStates.MEMORY][entity.id] = entity.value;
            }
        }
        catch (error) {
            BlisDebug_1.BlisDebug.Error(error);
    ...
    BlisMemory.prototype.WasRemembered = function (entityId) {
        return this.memory[Consts_1.UserStates.MEMORY][entityId];
    };
    /** Return array of entityIds for which I've remembered something */
    BlisMemory.prototype.EntityIds = function () {
        return Object.keys(this.memory[Consts_1.UserStates.MEMORY]);
    };
    //OBSOLETE
    ...
    BlisMemory.prototype.ForgetEntityById = function (entityId, entityValue) {
        try {
            ...
            if (entityName.startsWith(Consts_2.ActionCommand.BUCKET)) {
                ...
                var lowerCaseNames = this.memory[Consts_1.UserStates.MEMORY][entityId].map(function (value) {
                    return value.toLowerCase();
                });
                var index = lowerCaseNames.indexOf(entityValue.toLowerCase());
                if (index > -1) {
                    this.memory[Consts_1.UserStates.MEMORY][entityId].splice(index, 1);
                    if (this.memory[Consts_1.UserStates.MEMORY][entityId].length == 0) {
                        delete this.memory[Consts_1.UserStates.MEMORY][entityId];
                    }
                }
            }
            else {
    ...
    BlisMemory.prototype.ForgetEntity = function (entity) {
        try {
            ...
            if (entity.metadata && entity.metadata.bucket) {
                // Find case insensitive index
                var lowerCaseNames = this.memory[Consts_1.UserStates.MEMORY][positiveId].map(function (value) {
                    return value.toLowerCase();
                });
                var index = lowerCaseNames.indexOf(entity.value.toLowerCase());
                if (index > -1) {
                    this.memory[Consts_1.UserStates.MEMORY][positiveId].splice(index, 1);
                    if (this.memory[Consts_1.UserStates.MEMORY][positiveId].length == 0) {
                        delete this.memory[Consts_1.UserStates.MEMORY][positiveId];
                    }
                }
            }
            else {
                var positiveId_1 = entity.metadata.positive;
                delete this.memory[Consts_1.UserStates.MEMORY][positiveId_1];
            }
        }
        catch (error) {
            BlisDebug_1.BlisDebug.Error(error);
    ...
    BlisMemory.prototype.RememberLastStep = function (saveStep, value) {
        if (this.memory[Consts_1.UserStates.LASTSTEP] == null) {
            this.memory[Consts_1.UserStates.LASTSTEP] = new TrainStep();
        }
        if (saveStep == Consts_1.SaveStep.RESPONSES) {
            // Can be mulitple Response steps
            this.memory[Consts_1.UserStates.LASTSTEP][Consts_1.SaveStep.RESPONSES].push(value);
        }
        else if (saveStep = Consts_1.SaveStep.APICALLS) {
            // Can be mulitple API steps
            this.memory[Consts_1.UserStates.LASTSTEP][Consts_1.SaveStep.APICALLS].push(value);
        }
        else {
            this.memory[Consts_1.UserStates.LASTSTEP][saveStep] = value;
        }
    };
    BlisMemory.prototype.LastStep = function (saveStep) {
        if (!this.memory[Consts_1.UserStates.LASTSTEP]) {
            return null;
        }
        return this.memory[Consts_1.UserStates.LASTSTEP][saveStep];
    };
    //--------------------------------------------------------
    // TRAIN STEPS
    //--------------------------------------------------------
    BlisMemory.prototype.RememberTrainStep = function (saveStep, value) {
        if (!this.memory[Consts_1.UserStates.CURSTEP]) {
            this.memory[Consts_1.UserStates.CURSTEP] = new TrainStep();
        }
        var curStep = this.memory[Consts_1.UserStates.CURSTEP];
        if (saveStep == Consts_1.SaveStep.INPUT) {
            ...
            // Can be mulitple Response steps
            curStep[Consts_1.SaveStep.RESPONSES].push(value);
        }
        else if (saveStep = Consts_1.SaveStep.APICALLS) {
            // Can be mulitple API steps
            curStep[Consts_1.SaveStep.APICALLS].push(value);
        }
        else {
    ...
    BlisMemory.prototype.FinishTrainStep = function () {
        if (!this.memory[Consts_1.UserStates.TRAINSTEPS]) {
            this.memory[Consts_1.UserStates.TRAINSTEPS] = [];
        }
        var curStep = this.memory[Consts_1.UserStates.CURSTEP];
        this.memory[Consts_1.UserStates.TRAINSTEPS].push(curStep);
        this.memory[Consts_1.UserStates.CURSTEP] = null;
    };
    ...
    BlisMemory.prototype.TrainSteps = function () {
        return this.memory[Consts_1.UserStates.TRAINSTEPS];
    };
    ...
    BlisMemory.prototype.SetCueCommand = function (cueCommand) {
        this.memory[Consts_1.UserStates.CUECOMMAND] = cueCommand;
    };
    BlisMemory.prototype.CueCommand = function () {
        return this.memory[Consts_1.UserStates.CUECOMMAND];
    };
    //--------------------------------------------------------
    // Page
    //--------------------------------------------------------
    BlisMemory.prototype.SetPage = function (page) {
        this.memory[Consts_1.UserStates.PAGE] = page;
    };
    BlisMemory.prototype.NextPage = function () {
        var page = this.memory[Consts_1.UserStates.PAGE];
        page.index++;
        return page;
    };
    BlisMemory.prototype.PrevPage = function () {
        var page = this.memory[Consts_1.UserStates.PAGE];
        page.index--;
        return page;
    };
    ...
    BlisMemory.prototype.AppId = function () {
        return this.memory[Consts_1.UserStates.APP];
    };
    BlisMemory.prototype.ModelId = function () {
        return this.memory[Consts_1.UserStates.MODEL];
    };
    BlisMemory.prototype.DumpEntities = function () {
        var memory = "";
        for (var entityId in this.memory[Consts_1.UserStates.MEMORY]) {
            if (memory)
                memory += " ";
            var entityName = this.EntityId2Name(entityId);
            var entityValue = this.memory[Consts_1.UserStates.MEMORY][entityId];
    ...
    BlisMemory.prototype.Dump = function () {
        var text = "";
        text += "App: " + this.memory[Consts_1.UserStates.APP] + "\n\n";
        text += "Model: " + this.memory[Consts_1.UserStates.MODEL] + "\n\n";
        text += "Session: " + this.memory[Consts_1.UserStates.SESSION] + "\n\n";
        text += "InTeach: " + this.memory[Consts_1.UserStates.TEACH] + "\n\n";
        text += "InDebug: " + this.memory[Consts_1.UserStates.TEACH] + "\n\n";
        text += "LastStep: " + JSON.stringify(this.memory[Consts_1.UserStates.LASTSTEP]) + "\n\n";
        text += "Memory: {" + this.DumpEntities() + "}\n\n";
        text += "EntityLookup: " + JSON.stringify(this.memory[Consts_1.UserStates.ENTITYLOOKUP]) + "\n\n";
        return text;
    };
    return BlisMemory;
}());
