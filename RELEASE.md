...
If the code changes to @conversationlearner/sdk do not require changes in @@conversationlearner/ui or @conversationlearner/models then no special action is needed. Simply submit a PR and the merge will automatically publish a new version. Then update @conversationlearner/sdk-sample to consume this new version.
...
`@conversationlearner/sdk` is the only package we expect consumers to use and by default it is published to the `next` tag to allow publishing releases of newere possibly breaking features without risking / distrupting current user workflow.
