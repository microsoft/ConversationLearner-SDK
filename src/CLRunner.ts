...
export class CLRunner {
    ...
    // LUIS entity conflicts are found in the training status code for the application
    private hasLabelDiscrepancy(trainingError: any, trainDialog: CLM.TrainDialog, roundIndex: number): string | null {

        if (trainingError) {
            if (trainDialog.trainDialogId === trainingError.trainDialogId1 && roundIndex === trainingError.roundIndex1) {
                return trainingError.entityId
            }
            else if (trainDialog.trainDialogId === trainingError.trainDialogId2 && roundIndex === trainingError.roundIndex2) {
                return trainingError.entityId
            }
        }
    ...
    public async GetHistory(trainDialog: CLM.TrainDialog, userName: string, userId: string, clMemory: CLMemory): Promise<CLM.TeachWithHistory | null> {
        ...
        let trainingError = CLM.parseTrainingError(app)
        ...
        for (let [roundNum, round] of trainDialog.rounds.entries()) {
            ...
            // Check for conflicing entity labels
            let entityId = this.hasLabelDiscrepancy(trainingError, trainDialog, roundNum)
            if (entityId) {
                const errorEntity = entities.find(e => e.entityId === entityId)
                if (errorEntity) {
                    highlight = "error";
                    replayError = new CLM.ReplayErrorLabelDiscrepancy(errorEntity.entityName)
