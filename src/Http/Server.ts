    ...
    public static Init(): void {
        ...
        this.server.post('/app/:appId/teachwithhistory', async (req, res, next) => {
            try {
                ...
                // Get history and replay to put bot into last round
                let memory = BlisMemory.GetMemory(key)
                let teachWithHistory = await Blis.GetHistory(
                    appId,
                    trainDialog,
                    userName,
                    userId,
                ...
                if (teachWithHistory.discrepancies.length == 0) {
                    ...
                    if (teachWithHistory.dialogMode === DialogMode.Scorer) {
                        // Call LUIS callback
                        teachWithHistory.scoreInput = await Blis.CallEntityDetectionCallback(
                            '',
                            [],
                            memory,
                            trainDialog.definitions.entities
                        )
                        teachWithHistory.scoreResponse = await BlisClient.client.TeachScore(
                            appId,
